    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[repeat(auto-fit,minmax(16rem,1fr))]">
        {columns.map((column) => (
          <div
            key={column.status}
            className="flex min-h-36 min-w-0 flex-col rounded-[var(--admin-radius)] border border-white/15 bg-white/[0.03]"
          >
            ...
            <div className="flex-1 space-y-2 p-2">
              {column.cards.length === 0 ? (
                <p className="px-2 py-3 text-center text-sm text-[var(--admin-muted)]">
                  Empty
                </p>
              ) : (
